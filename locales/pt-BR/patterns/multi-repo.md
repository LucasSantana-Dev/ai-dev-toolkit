...
forge-patterns (core library)
    ↓
siza-gen (code generator, uses forge-patterns)
    ↓
siza (web app, uses siza-gen)
```

**Release order (bottom-up):**
```
1. forge-patterns: v1.5.0 (new feature)
2. siza-gen: v0.8.0 (uses forge-patterns@^1.5.0)
3. siza: v0.30.0 (uses siza-gen@^0.8.0)
```

**Wrong order (breaks everything):**
```
1. siza: import { newFeature } from 'siza-gen'
   ERROR: newFeature doesn't exist in siza-gen@0.7.0

2. siza-gen: import { helper } from 'forge-patterns'
...
repos=("forge-patterns" "siza-gen" "siza")
...
**Without context:**
```
You: "Update siza to use new assembleContext API from siza-gen"
...
gh pr create --title "feat: add new API" --body "New API for consumers. See siza-gen#123 for usage."
```
...
"Related: siza-gen#123 (consumer)"

In siza-gen#123 description:
...
npm publish
cd ~/siza
npm install @forgespace/siza-gen@latest
npm version minor
npm publish
...
jobs:
  test-consumers:
    ...
    steps:
      ...
      - name: Test against siza-gen
        run: |
          git clone https://github.com/org/siza-gen ../siza-gen
          cd ../siza-gen
          npm ci
          npm link ../forge-patterns
          npm test

      - name: Test against siza
        run: |
          git clone https://github.com/org/siza ../siza
          cd ../siza
          npm ci
          npm link ../forge-patterns
          npm test
...
**Bad:**
```
forge-patterns depends on siza-gen
siza-gen depends on forge-patterns
...
- siza-gen: uses forge-patterns (depends on patterns)
- siza: uses both (depends on patterns + siza-gen)

Dependencies flow one direction: siza → siza-gen → forge-patterns
```
...
siza-gen: v0.8.0 (uses patterns@^1.0.0)
...
2. siza-gen: Use metadata in assembler
3. siza: Display metadata in UI

Dependency order:
1. forge-patterns v1.5.0 (types)
2. siza-gen v0.8.0 (uses patterns@1.5.0)
3. siza v0.30.0 (uses siza-gen@0.8.0)
```
...
cd ~/siza-gen && npm link @forgespace/patterns
...
# Link local siza-gen and forge-patterns
cd ~/forge-patterns && npm link
cd ~/siza-gen && npm link
cd ~/siza && npm link @forgespace/patterns && npm link @forgespace/siza-gen

# Implement UI
git commit -m "feat: display component metadata"
git push origin feature/display-metadata
gh pr create --title "feat: display metadata" --body "Depends on: Forge-Space/siza-gen#456"
```
...
# 2. Update siza-gen dependency, merge, release
cd ~/siza-gen
...
# 3. Update siza dependencies, merge, release
cd ~/siza
npm unlink @forgespace/patterns @forgespace/siza-gen
npm install @forgespace/patterns@^1.5.0 @forgespace/siza-gen@^0.8.0
git add package.json package-lock.json
...
```bash
# Bug in forge-patterns affects siza-gen and siza
...
# siza
cd ~/siza
npm install @forgespace/patterns@^1.5.1 @forgespace/siza-gen@^0.8.1
npm test
...
- siza-gen: v0.8.0 → v0.8.1
- siza: v0.30.0 → v0.30.1

Deploy siza v0.30.1 immediately."
```
